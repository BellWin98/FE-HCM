...
interface KakaoInAppBrowserNoticeProps {
  visible: boolean;
  onClose: () => void;
}

export const KakaoInAppBrowserNotice = ({ visible, onClose }: KakaoInAppBrowserNoticeProps) => {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-background/95 p-4">
      <div className="relative w-full max-w-sm space-y-4 rounded-lg border bg-card p-6 text-center shadow-lg">
        ...
        <p className="text-sm text-muted-foreground">
          카카오톡 인앱 브라우저에서는 일부 화면이 정상적으로 표시되지 않을 수 있어요. 오른쪽
          하단 메뉴(⋮ 또는 ⋯)에서 <b>다른 브라우저로 열기</b>를 선택하거나, 아래 버튼으로 링크를
          복사해 사파리(Safari)·크롬(Chrome)·삼성 인터넷에서 열어주세요.
        </p>

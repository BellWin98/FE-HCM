...
import { getLandingStats, type LandingStats } from "@/api/stats";
...
const SocialProofStats = ({
  ...
  const stat1 = useCountUp({
    target: isLoading ? 0 : totalExerciseProofs,
    duration: 5000,
  });
  const stat2 = useCountUp({
    target: isLoading ? 0 : activeRooms,
    duration: 5000,
  });
      ...
      <div className="container mx-auto max-w-6xl px-4">
        <div className="flex flex-col items-center justify-between gap-4 sm:flex-row">
          ...
          <HeroRevealItem delay={HERO_REVEAL_STAGGER} revealed={isInView}>
            <SocialProofStats
              totalExerciseProofs={displayStats.totalExerciseProofs}
              activeRooms={displayStats.activeRooms}

...
이슈/PR 작업을 마치고 커밋을 푸시한 뒤에는 Notion에 작업 내용을 기록합니다. Notion MCP(`mcp__notion__*`)를
사용합니다.

**기록 위치** — `제발하자 > 프로젝트 제발하자 > HCM > 작업 히스토리 > FE-HCM`
([링크](https://app.notion.com/p/3aaa5de8c46680a7b820cdacc8a600ea))

**작성 규칙**

- `FE-HCM` 페이지 본문에 내용을 직접 쓰지 **않습니다**. 작업 단위(이슈/PR)마다 **하위 페이지를 새로 생성**하고
  그 안에 내용을 작성합니다.
- 하위 페이지 제목: `YYYY-MM-DD {작업 제목}` (예: `2026-07-27 FCM 푸시 알림 개선 (백그라운드 복구·중복 구현 통합)`).
- 페이지 구성:
  1. **메타 표** — 리포지토리, 이슈 번호/링크, PR 번호/링크, 브랜치, 커밋 해시, 변경 규모(파일 수, +/- 라인).
     연관된 백엔드 작업이 있으면 해당 PR 링크도 함께 표기합니다.
  2. **한 줄 요약** — 이 작업이 무엇을 바꿨는지 한 문장으로.
  3. **변경 내용** — 항목별 섹션. *무엇을 바꿨는지*뿐 아니라 **왜 그렇게 했는지(근거·트레이드오프)**를 함께
     적어 나중에 다시 봐도 판단 근거가 남도록 합니다.
- 커밋 메시지를 그대로 옮기지 말고, 코드/diff를 읽고 맥락과 배경을 담아 일목요연하게 재구성합니다.
- API 키·토큰 등 비밀값은 문서에 옮기지 않습니다(공개 값이라도 원문 대신 설명으로 대체).
